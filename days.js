...
const DAYS = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'p', html: 'In August 2026 they shipped <strong>Prime Agent</strong>: a self-improving coding harness — a loop that reads its own transcript and rewrites its own instructions to do better next time. It reports 95.5% Best@1 on ARC-AGI-3 against a 95.4% human-expert baseline — but read the fine print: that is the <strong>public set</strong>, 183 levels across 25 games. ARC-AGI-3 holds back private environments as a hidden evaluation set, and the benchmark says the hidden set is the real score, and is much harder for humans and machines alike. The result is genuinely good. It is not "beat humans at ARC-AGI-3". It also wrote a Sega Genesis emulator and a Game Boy Color emulator from scratch. Tomorrow\'s page comes back to that, because of what it means for games.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'lab', id: 'news' },
      { t: 'note', title: 'How to read AI news without becoming a fool', html: 'Benchmark numbers come from the people selling the thing. A model "beating humans" almost always means beating a specific baseline on a specific test. Ask three questions — <em>who measured it, against what, and can I reproduce it?</em><br><br><strong>Live example, from the card above.</strong> Prime Agent\'s 95.5% is on ARC-AGI-3\'s public set: 183 levels across 25 games. The benchmark deliberately holds back private environments, says the hidden set is the score that counts, and reports it is much harder. So the honest sentence is "beat the human baseline on the public games", not "beat ARC-AGI-3". Same number, completely different claim — and that gap is where almost every AI headline lives.' },
